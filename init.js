//Include Files
var careerLogicFile = require('./careerLogic');
var fundingLogicFile = require('./fundingLogic');
var otherLogicFile = require('./otherLogic');
var functionsFile = require('./functions');
...
exports.stripWord = function(userImput)
{
	...
	//Prints Related Questions (Not really needed)

	//functionsFile.printRelatedQuestions(questionList);
	...
	var feedback = careerLogicFile.careerLogic(myKeywordMatchesArray);
	//alert(feedback);
	if (feedback[0] == "empty")
	{
		feedback = fundingLogicFile.fundingLogic(myKeywordMatchesArray);
		//alert(feedback);
	}
	if (feedback[0] == "empty")
	{
		feedback = otherLogicFile.otherLogic(myKeywordMatchesArray);
		//alert("Third");
	}
	//conversationArray.push(feedback);
	var matchedWords = [];
	var foundNums = feedback[1];
	for(k = 0; k < foundNums.length; k++)
	{
		var num = foundNums[k];
		matchedWords[k] = myKeywordsArray[num];
	}


	//Returns [parsed string, keywords that were found in the string, the answer]
	return [myArray, keywords, matchedWords]; //Return for testing purposes

	//return feedback; //To return the actual function
}
